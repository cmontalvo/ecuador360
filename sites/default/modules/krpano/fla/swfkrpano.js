...
SWFkrpanoMouseWheel.prototype={
  init:function(){
    if(!SWFkrpanoMouseWheel.instances){
      ...
      if(window.opera)window.attachEvent("onmousewheel",SWFkrpanoMouseWheel_wheelEvent);else window.onmousewheel=document.onmousewheel=SWFkrpanoMouseWheel_wheelEvent;
      if(SWFkrpanoMouseWheel.isMac||this.wmodefix){
        document.onmouseup=SWFkrpanoMouseWheel_upEvent;
        var a=window.onload;
        window.onload=
        typeof window.onload!="function"?SWFkrpanoMouseWheel_registerEvents_delayed:function(){
          a();
...
function SWFkrpanoMouseWheel_registerEvents(){
  var a=0,b=SWFkrpanoMouseWheel.instances.length;
  for(a=0;a<b;a++){
    var d=SWFkrpanoMouseWheel.instances[a].so,c=d.getAttribute("id");
    if(c=document[c]){
...
function SWFkrpanoMouseWheel_wheelEvent(a){
  ...
  var d=false;
  if(b){
    var c=0,f=SWFkrpanoMouseWheel.instances.length;
    for(c=0;c<f;c++){
      var e=SWFkrpanoMouseWheel.instances[c].so.getAttribute("id"),g=document[e];
...
c.embed=function(f){
  ...
  window.addEventListener("load",function(){
    ...
    else if(krpanojs_init(c)==
      false)h="LICENSE ERROR";
...
function createkrpanoSWFviewer(e,a,b,d,c){
  ...
  if(typeof deconcept!=="undefined")if(deconcept.SWFObjectUtil.getPlayerVersion().major>=9){
    var g=
    new SWFObject(e,a,b,d,"9.0.28",c);
...
function embedpano(e){
  e||(e={});
  var a=e.swf?e.swf:"krpano.swf",b=e.xml?e.xml:a.split(".swf").join(".xml"),d=e.id?e.id:"krpanoSWFObject",c=e.target?e.target:null,f=e.width?e.width:"100%";
...
a.isDevice=function(b){
  var d="all",c=navigator.userAgent.toLowerCase();
  ...
  var f=b.length;
  for(c=0;c<f;c++)if(d.indexOf(b[c])>=0)return true;return false
  };

a.addVariable=function(b,d){
  b=String(b).toLowerCase();
  if(b=="xml"||b=="pano")a.pxml=d;else a.pvars[b]=d
    };
...
a.embed=function(b){
  ...
  if(a.ptarget){
    ...
    if(b=="always"||b=="force"||b=="whenpossible"&&a.isHTML5possible()){
      ...
      for(d in a.pparams)typeof a.pparams[d]!=
        "function"&&b.addParam(d,a.pparams[d])
        }
        for(var c in a.pvars)typeof a.pvars[c]!="function"&&b.addVariable(c,a.pvars[c]);b.addVariable("xml",a.pxml);
    b.embed(a.ptarget)
